 ...
 */
/*
 * 传统归并排序使用先二分后调用递归函数的步骤，应用对象主要是普通数组和vector数组，
 * 这两者的共同点在于可以在O(1)的时间内找到中点。但分析list数据结构可知，寻找其中
 * 点需要O(N)复杂度，故不大适合使用传统归并排序的思想。后来不知哪位牛人想到了利用
 * 二进制的进位思想，结合一个list数组保存各个归并层次的结果，最终实现了非递归版的
 * 归并排序，此想法也可以用在普通数组和vector数组上，具体实现以后有时间再写。
 */
template<class T,class Alloc>
